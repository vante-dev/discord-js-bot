...
module.exports = {
    ...
    messageRun: async (client, message, args, settings) => {
        ...
        if (args.length < 1) return client.systemEmbed(message, message.translate("utility/bug:REPORT"));

        if (args.length < 3) return client.systemEmbed(message, message.translate("utility/bug:MORE"));
        ...
        .setFields(
            ...
            { name: 'Reported Bug', value: `${report}`, inline: false },
        ...
        const logChannel = client.getWebhook('Bugs');
    ...
    interactionRun: async (client, interaction, settings) => {
        ...
        await interaction.followUp({
            content: interaction.translate("utility/bug:SUCCESS")
        })
    },
};