const { Guild } = require('discord.js'), { debug } = require('../../Helpers/Logger');
...
module.exports = Object.defineProperties(Guild.prototype, {
	...
	fetchSettings: {
		value: async function () {
			const data = await this.client.database.findOne('Cluster', 'Guild', { guildID: this.id, guildName: this.name }) 

			if (!data) {
				await this.client.database.create('Cluster', 'Guild', { guildID: this.id, guildName: this.name })
				this.settings = await this.client.database.findOne('Cluster', 'Guild', { guildID: this.id, guildName: this.name }) 
			} else {
				this.settings = data
			}
		},
	...
	updateGuild: {
		value: async function (settings) {
			debug(`Guild: [${this.id}] updated settings`);
			if (this.settings.guildID) {
				await this.client.database.updateOne('Cluster', 'Guild', { guildID: this.id }, settings);
			} else {
				await this.client.database.create('Cluster', 'Guild', { guildID: this.id, guildName: this.name })
				await this.client.database.updateOne('Cluster', 'Guild', { guildID: this.id }, settings)
			}
	...
	*/
	deleteDB: {
		value: async function () {
			try {
				await this.client.database.delete('Cluster', 'Guild', { guildID: this.id })
				return true;
			} catch (err) {
				return false;
			}
