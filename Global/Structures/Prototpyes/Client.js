...
const { Client: { Monitor } } = require("../../System");
        ...
        value: function (interaction, data, cooldown) {
            ...
            if (!cooldown) {
                if (interaction.deferred) {
                    ...
                    interaction.followUp({ embeds: [embed], ephemeral: true, deletable: false }).then(x => {
                        setTimeout(() => {
                    ...
                    interaction.reply({ embeds: [embed], ephemeral: true, deletable: false }).then(x => {
                        setTimeout(() => {
                            if (x && x.deletable) x.delete().catch(err => { })
                        }, 15 * 1000);
                    }).catch((e) => { });
